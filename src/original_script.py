    ...
    def draw(self, window, show, move_x, move_y, draw_line):
        ...
        if not self.sun:
            distance_text = FONT_2.render(
                f"{round(self.distance_to_sun * 1.057 * 10 ** -16, 8)} light years",
                True,
                COLOR_WHITE,
            )
            if show:
                window.blit(
                    distance_text,
                    (
                        x - distance_text.get_width() / 2 + move_x,
...
def main():
    ...
    earth = Planet(
        -1 * Planet.AU, 0, 10 * Planet.SCALE * 10**9, COLOR_EARTH, 5.9722 * 10**24
    )
    earth.y_vel = 29.783 * 1000

    mars = Planet(
        -1.524 * Planet.AU, 0, 5 * Planet.SCALE * 10**9, COLOR_MARS, 6.39 * 10**23
    )
    ...
    while run:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT or (
                ...
                and (event.key == pygame.K_x or event.key == pygame.K_ESCAPE)
            ):
                run = False
